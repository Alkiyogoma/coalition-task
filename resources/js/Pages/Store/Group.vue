<template>
  ...
  >
    <div class="flex items-center">
      ...
      <span class="ml-3">School Items Group</span>
    </div>
    <span class="mr-3"> Add Item  &RightArrow;</span>
  </Link>
    ...
    <table class="w-full whitespace-no-wrap" v-if="!items.length">
      <thead>
        <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
          ...
          <th class="px-2 py-2">Item Actions</th>
      </tr>
      </thead>
      <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
      <tr v-for="classe in items.data" :key="classe.id" class="text-gray-700 dark:text-gray-400">
        <td class="px-2 py-2">
          {{ classe.id }}
      </td>
        <td class="px-2 py-2">              
      ...
      <td class="px-2 py-2">
        <Link class="flex items-center px-2 py-2 focus:text-indigo-500" :href="`/classes/${classe.id}/view`">
          {{ classe.warehouse }}
        </Link>
      </td>
      <td class="px-2 py-2">
      ...
      <td class="px-2 py-2">
        <div class="flex items-center space-x-4 text-sm">
              <Link :href="`/usage/${classe.uuid}`" tabindex="-1" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="View">
            ...
            <span class="ml-2">Usages</span>
          </Link>
          <Link :href="`/items/${classe.uuid}`" tabindex="-1" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="View">
            <svg
                  class="w-5 h-5"
                  aria-hidden="true"
                  fill="none"
            ...
            <span class="ml-2">Items</span>
              ...
              <Link :href="`/editstore/${classe.uuid}/edit`" tabindex="-1"
              ...
              <Link :href="`/deletestore/${classe.uuid}/delete`" tabindex="-1"
                class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
  ...
  watch: {
    form: {
      ...
      handler: throttle(function () {
        this.$inertia.get('/itemgroups', pickBy(this.form), { preserveState: true })
      }, 150),
    },
  },
  methods: {
