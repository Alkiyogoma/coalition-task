<template>
  ...
  >
    <div class="flex items-center">
      ...
      <span class="ml-3"> List of School Items</span>
    </div>
    ...
    <table class="w-full whitespace-no-wrap" v-if="!items.length">
      <thead>
        <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
          ...
          <th class="px-2 py-2">Measure</th>
          <th class="px-2 py-2">Added</th>
          <th class="px-2 py-2">Used</th>
          <th class="px-2 py-2">Remained</th>
      ...
      <td class="px-2 py-2">
          <Link :href="`/purchase/${item.uuid}/view`" tabindex="-1" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="View">
            {{ item.added }}
          </Link>
      </td>
      
      <td class="px-2 py-2">
        <Link :href="`/usage/${item.uuid}/view`" tabindex="-1" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="View">
            {{ item.used }}
          </Link>
      </td>

      <td class="flex items-center px-2 py-2">
         <span class="flex items-center justify-between px-2 py-2 text-sm font-medium">  {{ item.balance }} </span>
        </td>
      <td class="px-2 py-2">
        <div class="flex items-center space-x-4 text-sm">
          <Link :href="`/usage/${item.uuid}/view`" tabindex="-1" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="View">
            <svg
              ...
              <Link :href="`/editstore/${item.uuid}/edit`" tabindex="-1"
  ...
  watch: {
    form: {
      ...
      handler: throttle(function () {
        this.$inertia.get('/items', pickBy(this.form), { preserveState: true })
      }, 150),
    },
  },
  methods: {
