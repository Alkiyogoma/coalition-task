<template>
    ...
    onclick="javascript:printDiv('printablediv')">
        <div class="flex items-center">
            ...
            <span> {{ payment.vendor }} Item Register</span>
        </div>
    ...
    <!-- <div class="invoice-box"> -->
        <table class="w-full whitespace-no-wrap" cellpadding="0" cellspacing="0">
            <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                ...
                <th class="px-2 py-2" align="left">
                   {{ payment.name }} .<br />
                    Phone - {{ payment.contact }} <br />
                    Reference - {{ payment.reference }}<br />
                </th>
                <th class="px-2 py-2" align="right">
                    Receipt #: 00{{ payment.reference }}<br />
                    Payment date: {{ payment.date }}<br />
                    Added Date: {{ payment.add_date }}<br />
                    <!-- Due: February 1, 2015 -->
                </th>
                </tr>
              
                <tr class="text-gray-700 dark:text-gray-400">
                    <th colspan="2"  class="px-2 py-2">{{ title }}</th>
                </tr>
                ...
                <tr  class="text-gray-700 dark:text-gray-400">
                    <td colspan="2" class="px-2 py-2">
                        <table class="w-full whitespace-no-wrap">
                            ...
                            <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                                <tr v-for="product in payment.items" class="text-gray-700 dark:text-gray-400">
                                    <td class="px-2 py-2">{{ product.code }}</td>
                                    <td class="px-2 py-2">{{ product.name }}</td>
                                    <td class="px-2 py-2">{{ product.quantity }}</td>
                                    <td class="px-2 py-2">{{ product.amount }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </td>
                ...
                <tr class="text-gray-700 dark:text-gray-400">
                    <td class="px-2 py-2">Total Quantity</td>
                    <td class="px-2 py-2">{{ payment.total }}</td>
                </tr>
                <tr class="text-gray-700 dark:text-gray-400">
                ...
                <tr class="text-gray-700 dark:text-gray-400">
                    <th colspan="2"  class="px-2 py-2">Comments - {{ payment.comment }}</th>
                </tr>
                <!-- <tr class="item last">
